<?php
if ( ! defined( 'ABSPATH' ) ) {
	/***** Closed if access directly. *****/
    exit; 
}

 /***** Create Api involve with users  *****/
 if( !class_exists( 'swp_endpoints_hide_on_mobile' ) ) {
    ...
    //    }
        ...
        /***** Get post views return category id *****/
        function swp_product_category_thumbnail_callback($post, $field_name, $response) {

            return get_term_meta($post['id'], $field_name);
        }
        
        /***** callback thumbnail url and return image url path ******/
