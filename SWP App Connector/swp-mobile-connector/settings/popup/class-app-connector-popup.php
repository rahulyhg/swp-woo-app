...
if( !class_exists( 'SWPapppopup' ) ){
    ...
    class SWPapppopup{
        ...
        public function __construct(){
            ...
            add_action( 'admin_post_app_update_popup', array( $this, 'swp_popup_update' ) );
        ...
        public function swp_popup_update(){
            ...
            check_admin_referer( "app_update_popup" );

            $capability = apply_filters( 'apppopup_capability', 'edit_others_posts' );
            ...
            $link = @$_POST['swp_popup_link'];

            if(!empty($link)){

                update_option('swp-popup-homepage-link',$link);

            }else{

                update_option('swp-popup-homepage-link','');

            }

            $check = @$_POST['swp_check_popup_datetime'];

            if(!empty($check) && $check == 1){

                update_option('swp-popup-homepage-check',$check);

                $from = @$_POST['swp_popup_datepicker_from'];

                $to = @$_POST['swp_popup_datepicker_to'];

                update_option('swp-popup-homepage-date-from',$from);

                update_option('swp-popup-homepage-date-to',$to);

            }else{

                update_option('swp-popup-homepage-check','');

                update_option('swp-popup-homepage-date-from','');

                update_option('swp-popup-homepage-date-to','');
            ...
            if(!empty($attachments)){
                ...
                update_option('swp-popup-homepage',$attachments);

            }else{

                update_option('swp-popup-homepage','');
        ...
        public function swp_app_popup_action(){
            ...
            $task = isset($_REQUEST['apptask']) ? $_REQUEST['apptask'] : '';			
