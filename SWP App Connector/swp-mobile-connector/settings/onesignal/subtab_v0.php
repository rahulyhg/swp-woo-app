...
<h2 class="nav-tab-wrapper" <?php if($wp_version < '4.8'){echo 'style="border-bottom: 1px solid #ccc"';}  ?>>
	<a id="wooconnector-onesignal-settings-api" href="?page=swp-one-signal&amp;wootab=api" class="nav-tab nav-tab-general<?php echo (($active_tab == 'api') ? ' nav-tab-active' : ''); ?>"><?php echo __('Onesignal API','swp');?></a>
	<a id="wooconnector-onesignal-push-new" href="?page=swp-one-signal&amp;wootab=new" class="nav-tab nav-tab-template<?php echo (($active_tab == 'new') ? ' nav-tab-active' : ''); ?>"><?php echo __('New Notification','swp');?></a>
	<a id="wooconnector-onesignal-list-send" href="?page=swp-one-signal&amp;wootab=list" class="nav-tab nav-tab-template<?php echo (($active_tab == 'list') ? ' nav-tab-active' : ''); ?>"><?php echo __('Sent Notification','swp');?></a>
	<a id="wooconnector-onesignal-all-users" href="?page=swp-one-signal&amp;wootab=player" class="nav-tab nav-tab-template<?php echo (($active_tab == 'player') ? ' nav-tab-active' : ''); ?>"><?php echo __('All Users','swp');?></a>		
</h2>
...
<?php
	...
	if($attab != 'api'){	
		global $wpdb;
		if(get_option('wooconnector_settings-api')){
			$api = get_option('wooconnector_settings-api');
		}else{
			$api = 0;
		}
		if(get_option('wooconnector_settings-restkey')){
